...
def key_expansion(key, rounds):
    """
    Expand the key into round keys for AES-128, AES-192, or AES-256.
    """
    expanded_key = list(key)
    key_size_words = len(key) // 4  # Number of 4-byte words in the key

    for i in range(key_size_words, 4 * (rounds + 1)):
        temp = expanded_key[-4:]
        if i % key_size_words == 0:
            temp = sub_bytes(rot_word(temp))
            temp = bytes([temp[0] ^ r_con[i // key_size_words - 1]] + list(temp[1:]))
        elif key_size_words > 6 and i % key_size_words == 4:
            temp = sub_bytes(temp)
        temp = bytes([temp[j] ^ expanded_key[-key_size_words * 4 + j] for j in range(4)])
        expanded_key += temp
...
def encrypt_block(state: bytes, expanded_key: bytes, rounds: int):
    ...
    for i in range(middle_rounds):
        print(f"Round {i + 1}")
        print("start=", bytes_to_hex_string(state))
        # Sub Bytes
        ...
        # Key Iter
        key = next(round_key_gen)
        print("k_sch=", bytes_to_hex_string(key))
...
def decrypt_block(state: bytes, expanded_key: bytes, rounds=11):
    ...
    for i in range(middle_rounds):
        print(f"Round {i + 1}")
        print("i_start=", bytes_to_hex_string(state))
        state = inv_shift_rows(state)
        print("is_row=", bytes_to_hex_string(state))
        state = inv_sub_bytes(state)
        print("is_box=", bytes_to_hex_string(state))
        key = next(round_key_gen)
        print("ik_sch=", bytes_to_hex_string(key))
        state = add_round_key(state, key)
        print("ik_add=", bytes_to_hex_string(state))
    ...
    state = add_round_key(state, key)
    print("ik_sch=", bytes_to_hex_string(key))
    print("ioutput=", bytes_to_hex_string(state))
    return state
...
def aes_decrypt(message: bytes, key: bytes, mode=None, iv: bytes = None, rounds=11):
    expanded_key = key_expansion(key, rounds)
...
with open("input.txt", "r") as inp_file:
    plain_text = bytes.fromhex(inp_file.read())

# Сам алгоритм
bts = aes_encrypt(plain_text, key, rounds=15)

# Запись зашифрованного ответа в файл
with open("output.txt", "w") as out_file:
    out_file.write(bytes_to_hex_string(bts))